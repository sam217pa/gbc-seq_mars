...
#+BEGIN_SRC sh :results none :shebang "#!/usr/bin/env bash" :tangle no
muscle \
    -profile \
    -quiet \
...
do
    phruscle basecall \
			       --input $file \
			       --ref $ROOT/raw/ref/aln-w-ref.fst \
             --cutoff 0.10
...
do
    phruscle basecall \
			       --input $file \
			       --ref $ROOT/raw/ref/aln-s-ref.fst \
             --cutoff 0.10
...
*** analyses complémentaires [6/6]
Aujourd'hui [2016-04-15 Fri] nous avons discuté avec Laurent, Vincent et Franck
de la façon optimale de conduire la suite des analyses.

On s'est rendu compte de plusieurs choses.

1. [X] On se demande où est partie la première (ou la dernière ?) position de SNP
   ...
   premier SNP.
2. il faut essayer de représenter sur un graphe les effectifs de point de
   bascule.
3. [X] On peut avoir différentes infos : on peut compter les doublets SS et WW dans
   ...
   résumer ça par la pseudo équation =nombre de doublet total = nombre de
   doublet en fin de conversion + nombre de restauration du genotype parental=.
4. [X] sur la figure des alignements, il serait judicieux de représenter quelque
   part le génotype parental, et le génotype du donneur.
5. [X] On veut également représenter la trace de conversion par du bleu tout le
   temps, et le génotype parental par du rouge. On aurait de ce fait les mêmes
   codes couleurs pour toutes les manips.
6. [X] pour compter les cas de tracts complexes, /on ne doit pas filtrer/ sur la
   ...
   qualité par des N.
7. [X] Pour différencier les cas de restauration des cas "normaux", on peut
   représenter les points, en gardant le même code couleur (donneur / receveur)
   par des ~+~ ou des carrés.
8. On peut éviter de parler de S, W, SW et WS, mais parler de GC+, AT+ etc… Pour
