...
#+BEGIN_SRC sh :results none :shebang "#!/usr/bin/env bash" :tangle src/make_ref
muscle \
    -profile \
    -quiet \
...
1. On se demande où est partie la première (ou la dernière ?) position de SNP
...
2. il faut essayer de représenter sur un graphe les effectifs de point de
   bascule.
3. On peut avoir différentes infos : on peut compter les doublets SS et WW dans
...
4. sur la figure des alignements, il serait judicieux de représenter quelque
   ...
   toutes les manips.
5. pour compter les cas de tracts complexes, /on ne doit pas filtrer/ sur la
   ...
   qualité par des N.
6. Pour différencier les cas de restauration des cas "normaux", on peut
   représenter les points, en gardant le même code couleur (donneur / receveur)
   par des ~+~ ou des carrés.
7. On peut éviter de parler de S, W, SW et WS, mais parler de GC+, AT+ etc… Pour
