...
def is_abi(sequence):
    ...
    with open(sequence, "rb") as input:    # le fichier doit être ouvert en mode "rb"
        if len(SeqIO.read(input, "abi").id) > 0:  # teste la longueur de l'id de la séquence
            return True
        else:
            return False
...
def is_fasta(sequence):
    ...
    with open(sequence, "rb") as input:
        ...
        for record in SeqIO.parse(input, "fasta"):
            seq_list.append(len(record.id))

    if 0 not in seq_list :
        return True
    else:
        return False
...
def run_phred(input, cutoff):
    ...
    phred = [
        "phred",
        "-st", "fasta",          # sequence type output (default = fasta)
        "-trim_alt", "\"\"",     # quality trim
        "-trim_cutoff", str(cutoff),  # error probability of trimming
        ## TODO voir si ça ne serait pas intéressant de ne trimmer que les sorties qui ne nous
        ## intéressent pas. Genre phd et fasta et compagnie.
        "-trim_fasta",           # trim sortie fasta.
        "-trim_phd",             # trim sortie phd
        # "-c",                    # write scf file with the trace data
        "-s",                    # write seq file, append ".seq" to the names of the input files
        "-d",                    # write a poly file
        "-p",                    # write a phd file
        "-if", "tempfile" # input file
    ]
    phred_call = sub.call(phred)
    os.remove("tempfile")       # supprime le fichier temporaire
    return phred_call

# (X) "phred -st fasta -trim_alt \"\" -trim_cutoff $c -trim_out -c $t.scf -s -d -p -if $tempFile"; la commande perl dans polysnp.
# (X) run_phred(test_seq)
...
def run_muscle(input, reference):
    ...
    muscle_call = sub.call(muscle)
    return muscle_call

# (X) run_muscle("pW85-1073.ab1", "aln-weak-wt.fst")
...
parse_muscle(test_seq + ".aln")[0][1][1]

def index_phd(align):
    # sequence = parse_muscle(align)
sequence = parse_muscle(test_seq + ".aln")

for base in sequence[0][1]:
    print base

for i in sequence[0][1]:
    print i

# (X) def read_seq(sequence, index):
# (X)     return AlignIO.read(sequence, "fasta")[index]

# (X) print AlignIO.read("pW85-1073.ab1.aln", "fasta")[1].id

print read_seq(test_seq + ".aln", 0).seq
#+END_SRC
